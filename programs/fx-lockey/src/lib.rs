...
declare_id!("Dw2GQsEseYiZkccZfJoBwXu3kJ2hK3rgu7ERfYpB7fUE");

#[program]
mod fx_lockey {
    use super::*;
    pub fn init_mint(_ctx: Context<InitMint>, _bump: u8) -> Result<()> {
        Ok(())
    }
    pub fn create_deposit_vault(_ctx: Context<InitDepoVault>, _bump: u8) -> Result<()> {
        Ok(())
    }

    pub fn stake_tokens(ctx: Context<StakeTokens>, bump: u8, amount: u64) -> Result<()> {
